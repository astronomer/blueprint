...
def _unpause_dag(api_client: httpx.Client, dag_id: str) -> None:
    resp = api_client.patch(f"/api/v2/dags/{dag_id}", json={"is_paused": False})
    assert resp.status_code == 200, f"Failed to unpause {dag_id}: {resp.text}"


def _trigger_dag(api_client: httpx.Client, dag_id: str) -> str:
...
def _wait_for_dag_run(api_client: httpx.Client, dag_id: str, run_id: str) -> dict:
    deadline = time.monotonic() + DAG_RUN_TIMEOUT
    while time.monotonic() < deadline:
        resp = api_client.get(f"/api/v2/dags/{dag_id}/dagRuns/{run_id}")
...
class TestSimplePipelineExecution:
    def test_runs_successfully(self, api_client: httpx.Client):
...
class TestVersionedETLExecution:
    ...
    def test_all_tasks_succeed(self, api_client: httpx.Client):
        dag_id = "versioned_etl"
        _unpause_dag(api_client, dag_id)
        run_id = _trigger_dag(api_client, dag_id)
        result = _wait_for_dag_run(api_client, dag_id, run_id)
        ...
        resp = api_client.get(f"/api/v2/dags/{dag_id}/dagRuns/{run_id}/taskInstances")
        assert resp.status_code == 200
